...
public class PlayerControler : MonoBehaviour
{
	...
	private void ControlAngle() {
		if (transform.position.x > (ground.localScale.x - scale) / 2) {
			transform.position = new Vector3(
				(ground.localScale.x - scale) / 2,
				transform.position.y,
				transform.position.z);
		}
		else if (transform.position.x < -(ground.localScale.x - scale) / 2) {
			transform.position = new Vector3(
				-(ground.localScale.x - scale) / 2,
				transform.position.y, 
				transform.position.z);
		...
		if (transform.position.z > (ground.localScale.z - scale) / 2) {
			transform.position = new Vector3(
				transform.position.x,
				transform.position.y,
				(ground.localScale.z - scale)/2);
		}
		else if (transform.position.z < -(ground.localScale.z - scale) / 2) {
			transform.position = new Vector3(
				transform.position.x,
				transform.position.y,
				-(ground.localScale.z - scale)/2);
	...
	public void AddScore(int amount)
	{
		...
		GameManager.gm.ui.SetScore(score);
	}
